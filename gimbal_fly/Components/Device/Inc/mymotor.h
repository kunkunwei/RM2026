...
typedef enum
{
    ...
    CAN2_CMD_ALL_ID = 0,                 // CAN2发送命令帧ID(预留)

    CAN1_YAW_MOTOR_ID = 0x01,             // Yaw轴达妙电机ID
    CAN1_PITCH_MOTOR_ID = 0x02,        // Pitch轴电机ID
    CAN1_ROLL_MOTOR_ID =0x03            // Roll轴电机ID
} can_msg_id_e;
...
/* 达妙电机实例声明 */
extern dm_motor_measure_t yaw_motor;  // Yaw轴达妙电机
extern dm_motor_measure_t pitch_motor;  // Pitch轴达妙电机
extern dm_motor_measure_t roll_motor;  // Roll轴达妙电机

/* 达妙电机访问函数声明 */
dm_motor_measure_t* get_yaw_motor();  // 获取Yaw轴电机指针
dm_motor_measure_t* get_pitch_motor();  // 获取Yaw轴电机指针
dm_motor_measure_t* get_roll_motor();  // 获取Roll轴电机指针
