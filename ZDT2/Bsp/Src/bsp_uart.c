...
void BSP_USART_Init(void)
{
  /* Starts the remote control multi_buffer DMA Transfer with interrupt enabled. */
	...
	/* Starts the PC communication multi_buffer DMA Transfer with interrupt enabled. */
	USART_RxDMA_MultiBufferStart(&huart6,(uint32_t *)&(huart6.Instance->DR),(uint32_t *)PC_MultiRx_Buf[0],(uint32_t *)PC_MultiRx_Buf[1],PC_RX_BUF_SIZE);
}
//------------------------------------------------------------------------------
...
{	
  ...
  /* configuare the huart Receicve Size - 修复：使用实际的DataLength而不是固定值 */
...
/**
* @简要说明  用户 USART6 接收事件回调函数（上位机通信）
* @参数  huart UART 句柄
* @参数  Size 应用程序接收缓冲区中可用的数据数量
* @返回值  无
 */
static void USER_USART6_RxHandler(UART_HandleTypeDef *huart,uint16_t Size)
{
  ...
  if(((((DMA_Stream_TypeDef  *)huart->hdmarx->Instance)->CR) & DMA_SxCR_CT ) == RESET)
  {
    ...
    huart->hdmarx->Instance->CR |= DMA_SxCR_CT;
    // 重装计数器
    __HAL_DMA_SET_COUNTER(huart->hdmarx,PC_RX_BUF_SIZE);
    // 解析上位机数据包
    if(Size > 0)
    {
      PC_Packet_Parse(PC_MultiRx_Buf[0], Size, &pc_data);
    }
  }
  else
  {
    ...
    if(Size > 0)
    {
      PC_Packet_Parse(PC_MultiRx_Buf[1], Size, &pc_data);
    }
  }
