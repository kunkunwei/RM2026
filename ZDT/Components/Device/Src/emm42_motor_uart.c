 ...
 */
#define em42_huart &huart6
#define ABS(x) ((x) < 0 ? -(x) : (x))
Motor_FeedbackData yaw_motor_feedback =
  {
  .addr = 1,
  .enable = ENABLE,
  .error = 0,
  ...
  .stall = false,
  .ready = true,
  .type = MOTOR_FEEDBACK_NONE
  };
Motor_FeedbackData pitch_motor_feedback =  {
  .addr = 2,
  .enable = ENABLE,
  .error = 0,
  .position = 0,
...
{
  if (!rx || !data) return false;
  if (rx[len-1] != CHECKSUM_BYTE) return false;

  data->addr = rx[0];
  data->type = MOTOR_FEEDBACK_NONE;

  // 位置反馈：6字节，01 xx xx xx xx 6B
  if (len == 6 && rx[1] != 0xEE && rx[1] != 0x02 && rx[1] != 0x9F) {
    data->type = MOTOR_FEEDBACK_POSITION;
    data->position = ((int32_t)rx[1] << 24) | ((int32_t)rx[2] << 16) | ((int32_t)rx[3] << 8) | rx[4];
    return true;
  }
  // 误差反馈：4字节，01 xx xx 6B
  if (len == 4 && rx[1] != 0xEE && rx[1] != 0x02 && rx[1] != 0x9F) {
    data->type = MOTOR_FEEDBACK_ERROR;
    data->error = ((int16_t)rx[1] << 8) | rx[2];
    return true;
  }
  // 使能状态/堵转标志/命令确认/位置完成：3字节
  if (len == 3) {
    if (rx[1] == 0x00 || rx[1] == 0x01) {
      data->enable = rx[1];
      data->stall = rx[1];
      data->type = MOTOR_FEEDBACK_ENABLE; // 或 MOTOR_FEEDBACK_STALL
      return true;
    }
    // 命令确认
    if (rx[1] == 0x02 ) {
      data->type = MOTOR_FEEDBACK_CMD_ACK;
      data->ready = false; // 收到应答后，未完成
      return true;
    }
    // 位置命令完成
    if (rx[1] == 0x9F) {
      data->type = MOTOR_FEEDBACK_POS_DONE;
      data->ready = true; // 可以发送新命令
      return true;
    }
    // 错误
    if (rx[1] == 0xEE)
...
bool Motor_MoveRelative(uint8_t addr, int16_t speed, uint8_t acc, float angle)
{
  ...
  uint32_t pulse = (uint32_t)((ABS(angle) * 65536.0f) / 360.0f);
  uint8_t cmd[] = {
      0xFD,
      (uint8_t)(((direction & 0x0F) << 4) | ((abs_speed >> 8) & 0x0F)),
      (uint8_t)(abs_speed & 0xFF),
