...
void chassis_mode_change_control_transit(chassis_move_t *chassis_move_transit)
{
             ...
             chassis_move_transit->chassis_mode == CHASSIS_SPINNING_MODE)
    {
        ...
        chassis_move_transit->chassis_yaw_set = chassis_move_transit->chassis_yaw;
        chassis_move_transit->spining_state = true;
             ...
             chassis_move_transit->chassis_mode == CHASSIS_FORCE_RAW)
    {
        ...
        chassis_move_transit->jump_state.jump_stage = 0;
        chassis_move_transit->spining_state = false;
        chassis_move_transit->spining_flag = false;
    }
...
void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
    ...
    {
        // 小陀螺模式（云台正方向为机器人正方向）
        // 底盘会持续旋转，保持云台相对世界坐标系下的角度不变（IMU角度）
        // 除非遥控器控制云台转动（绝对角度转动）
        chassis_move_control->chassis_yaw_set += 0.008f;
        chassis_move_control->spining_state = true;
        ...
        if (!switch_is_down(chassis_move_control->chassis_RC->rc.s[1]))
        {
            // 小陀螺模式退出的时候，底盘按照转动最小的角度旋转到底盘正方向和云台正方向重合为止
            // 默认进入底盘跟随云台模式
            chassis_move_control->spining_state = false;
            chassis_move_control->chassis_mode = CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW;
...
void cacul_support(chassis_move_t *chassis_move_control_loop)
{
    ...
    if (chassis_move_control_loop->touchingGroung) // 正常触地状态
    {
        ...
        static const fp32 gravity_comp = 72.0f;
...
void roll_compensate(chassis_move_t *chassis_move_control_loop)
{
    ...
    // 设置目标腿长并进行最终限幅保护
    chassis_move_control_loop->left_leg.leg_length_set = fp32_constrain(left_target, LEG_LENGTH_MIN, LEG_LENGTH_MAX);
    chassis_move_control_loop->right_leg.leg_length_set = fp32_constrain(right_target, LEG_LENGTH_MIN, LEG_LENGTH_MAX);
}
...
static void Jump_stage_force_comp(chassis_move_t *c, uint8_t stage)
{
    ...
    // 修复BUG 3：使用独立的跳跃目标，而不是被Roll补偿修改过的leg_length_set
    ...
    case 4:
        ...
        break;
    default:
        target_left = c->left_leg.leg_length_set;
        target_right = c->right_leg.leg_length_set;
        break;
    }

    float e_l = target_left - c->left_leg.leg_length;
    float e_r = target_right - c->right_leg.leg_length;
...
void Jump_control(chassis_move_t *chassis_move_control_loop)
{
    ...
    {
        chassis_move_control_loop->left_leg.leg_length_in_sky_set = 0.39f;
        chassis_move_control_loop->right_leg.leg_length_in_sky_set = 0.39f;
        chassis_move_control_loop->left_leg.leg_length_set = 0.39f;
        chassis_move_control_loop->right_leg.leg_length_set = 0.39f;

        // 起跳方向管理：允许机器人保持起跳时的自然前倾
    ...
    if (chassis_move_control_loop->jump_state.jump_stage == 4)
    {
        ...
        chassis_move_control_loop->right_leg.leg_length_set = landing_buffer_length;
        chassis_move_control_loop->left_leg.leg_length_in_sky_set = landing_buffer_length;
        chassis_move_control_loop->right_leg.leg_length_in_sky_set = landing_buffer_length;
        ...
        bool_t force_stable = (fabsf(chassis_move_control_loop->left_support_force - 70.0f) < 15.0f) &&
                              (fabsf(chassis_move_control_loop->right_support_force - 70.0f) < 15.0f);
...
void LQR_Balance_Turn(chassis_move_t *chassis_move_control_loop)
{
    ...
    // 反馈矩阵乘以一定系数用于调整。因为平衡点已经找到了，所以不必每次机体质量增加的时候，都去重新生成、调节所有的矩阵系数，只需要乘以系数微调就行了。
    // coefficient负责机器人不同质量下的PD手动调参。默认为1，能丝滑控制10.8KG的机体。
    fp32 coefficient[2][6] = {{2.0f, 1.5f, 2.0f, 1.6f, 2.0f, 1.8f},//这个是装好裁判系统，(20.3kg)步兵完全体的参数。
