 ...
 * @note  static: 只允许本 TU 内 SBUS_TO_RC 写入，外部通过
 *        RC_Process_Buffer() 触发更新，通过 get_remote_control_point() 只读访问
 */
static Remote_Info_Typedef remote_ctrl={
	.rc_lost = true,
	.online_cnt = 0xFAU,
};
...
/**
 * @brief  处理一帧 DBUS/SBUS 遥控数据（由 BSP UART 回调调用）
 * @param  buf  DMA 接收缓冲区指针
 * @param  size 接收到的字节数
 * @note   封装了对内部 static remote_ctrl 的写操作，替代旧的
 *         SBUS_TO_RC(buf, &remote_ctrl) 调用方式
 */
void RC_Process_Buffer(volatile const uint8_t *buf, uint16_t size)
{
    if (size == RC_FRAME_LENGTH)
    {
        SBUS_TO_RC(buf, &remote_ctrl);
    }
}
//------------------------------------------------------------------------------
