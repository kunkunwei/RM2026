...
static dji_motor_measure_t shoot_motor_left, shoot_motor_right, pull_motor;  // 左右摩擦轮 + 拨弹电机
static dji_motor_measure_t yaw_motor;   // Yaw电机
/* 达妙电机实例定义 */
...
void get_dm_motor_measure(dm_motor_measure_t* ptr, uint8_t *rx_message)
{
    ...
    ptr->pos = uint_to_float(tmp_pos,P_MIN,P_MAX,16);  // 转换为弧度值(-π~π)
