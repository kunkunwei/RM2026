...
//////////////////////
// USE_SBUS_PROTOCOL 已移至 config.h 统一管理
//////////////////////////
...
///////////////////////////////
/* 发射系统参数设置 */
#define SHOOT_SPD 750.0f                    // 发射轮目标转速，750对应22-25m/s
#define SHOOT_CHANNEL_TO_SPEED 1.0 / 660.0f // 通道值转换为速度的系数（660->1rad）

/* 拨弹轮参数：角度用于判定一发，速度用于控制弹频 */
#define SHOOT_SINGLE_BULLET_RAD 2.5f * PI  // 单发拨弹轮累计转角阈值
...
/* YAW轴速度环PID参数 */
#define YAW_SPEED_PID_KP 2000.0f       // 比例系数
#define YAW_SPEED_PID_KI 6.0f          // 积分系数
#define YAW_SPEED_PID_KD 0.0f          // 微分系数
#define YAW_SPEED_PID_MAX_OUT 10000.0f // PID最大输出
#define YAW_SPEED_PID_MAX_IOUT 1000.0f // PID最大积分输出

/* YAW轴相对角度角度环PID参数 */
#define YAW_GYRO_RELATIVE_PID_KP 18.0f      // 比例系数
#define YAW_GYRO_RELATIVE_PID_KI 0.05f      // 积分系数
#define YAW_GYRO_RELATIVE_PID_KD 0.4f       // 微分系数
#define YAW_GYRO_RELATIVE_PID_MAX_OUT 12.0f // PID最大输出
#define YAW_GYRO_RELATIVE_PID_MAX_IOUT 1.2f // PID最大积分输出

/* YAW轴角度环PID参数（基于陀螺仪解算角度） */
#define YAW_GYRO_ABSOLUTE_PID_KP 18.0f      // 比例系数
#define YAW_GYRO_ABSOLUTE_PID_KI 0.05f      // 积分系数
#define YAW_GYRO_ABSOLUTE_PID_KD 0.4f       // 微分系数
#define YAW_GYRO_ABSOLUTE_PID_MAX_OUT 12.0f // PID最大输出
...
typedef enum
{
    ...
    // GIMBAL_MOD_SPINING_MODE = 6, // 小陋螺模式，底盘绕着一个方向不断旋转，并且不能移动，同时云台保持绝对角度位置不变
} gimbal_mod_e;

/**
 * @brief 底盘控制模式枚举
 * @details 云台通过 USART6 向底盘发送的动作模式指令
 */
typedef enum
{
    CHASSIS_FORCE_RAW = 0,                // 无力/原始模式
    CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW = 1, // 底盘跟随云台YAW
    CHASSIS_VECTOR_NO_FOLLOW_YAW = 2,     // 底盘不跟随 YAW（云台跟随底盘模式）
} chassis_mode_e;

/**
 * @brief 云台向底盘发送的控制指令结构体
 * @details 封装底盘通信所需的全部字段，由 gimbal_ctl_chassis_cmd() 填写
 */
typedef struct
{
    chassis_mode_e chassis_mode_cmd; // 底盘工作模式指令
    bool spinning_cmd;               // 小陋螺模式启用
    bool jump_cmd;                   // 跳跃指令
    float target_speed_x;            // 射击方向目标速度 (RC通道原始值)
    float target_speed_w_z;          // 旋转目标速度 (RC通道原始值)
    float target_length;             // 腿长目标值 (RC通道原始值)
    float roll_angle;                // Roll滚转角度 (rad)
    float gimbal_yaw_angle;          // 云台YAW绝对角度 (rad)，底盘跟随参考
    float gimbal_yaw_gyro;           // 云台YAW角速度 (rad/s)
...
typedef struct
{
    ...
    const PC_Ctrl_Info_t *gimbal_PC_RC; // 图传链路
    bool PC_fn1_last;                   // 上一次图传链路fn_1按键状态
    bool PC_fn2_last;                   // 上一次图传链路fn_2按键状态
    bool PC_pause_last;                 // 上一次图传链路pause按键状态
#else
#ifdef USE_SBUS_PROTOCOL
    ...
    bool anto_aim_mode_cmd;       // 自动矄准模式指令
    bool last_anto_aim_mode_cmd;  // 上一次自动矄准模式指令
    bool spinning_mode_cmd;       // 小陋螺模式指令
    bool last_spinning_mode_cmd;  // 上一次小陋螺模式指令
    bool spinning_mode;           // 小陋螺模式状态,小陋螺模式下底盘会绕着一个方向不断旋转，并且不能移动，同时云台保持绝对角度位置不变

    gimbal_chassis_cmd_t chassis_cmd; // 发射给底盘的控制指令字段
