...
void init_gimbal_shoot(gimbal_shoot_t* shoot){
    ...
    const static fp32 pull_w_d_pid[3] =    {PID_SHOOT_PULL_D_MOTOR_KP,PID_SHOOT_PULL_D_MOTOR_KI,PID_SHOOT_PULL_D_MOTOR_KD};
    ...
    old_PID_Init(&shoot->Pull_D_PID,      PID_DELTA,        pull_w_d_pid,PID_SHOOT_PULL_D_MOTOR_MAXOUT, PID_SHOOT_PULL_D_MOTOR_MAXI);
...
void pull_limit(gimbal_shoot_t* shoot_control)
{
    if (shoot_control->pull_motor->target_current>3000)
    {
        shoot_control->pull_motor->target_current = 3000;
    }
    else if (shoot_control->pull_motor->target_current<-3000)
    {
        shoot_control->pull_motor->target_current = -3000;
    }
}
void pull_block(gimbal_shoot_t* shoot_control)
{
    if (shoot_control->pull_motor->real_w > -2.0f && shoot_control->pull_motor->real_w < 2.0f&&(gimbal.gimbal_RC->rc.ch[4]<-300))
    {
        //如果拨弹电机速度在0.01f以内，说明拨弹电机处于堵转状态

        // old_PID_clear(&shoot_control->Pull_PID);
        shoot_control->pull_motor->target_current = -2000;
    }
    else
    {
        //如果拨弹电机速度不在0.01f以内，说明拨弹电机处于正常工作状态
        //此时需要将拨弹电机的速度设置为目标速度
...
void cal_gimbal_shoot_control(gimbal_shoot_t* shoot_control,float shoot_spd,bool_t shoot_flag,bool_t fire_flag){
    ...
    if (switch_is_down(gimbal.gimbal_RC->rc.s[0]))
        shoot_control->shoot_ready_flag == false;
