...
/*SkyAdd-Start*/
// PID计算，使用入参err作为PID计算所用数值，而不是get与set之差,方便先在外面对err做处理
static fp32 GIMBAL_PID_Calc_WithErr(Gimbal_PID_t *pid, fp32 get, fp32 set, fp32 err, fp32 error_delta);
/*SkyAdd-End*/
...
void GIMBAL_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        CAN_CMD_Fric(0, fric1_M_control.given_current, fric2_M_control.given_current, 0);     // fric1_M_control.given_current  fric2_M_control.given_current
        CAN_CMD_GIMBAL(Yaw_Can_Set_Current, Pitch_Can_Set_Current, Shoot_Can_Set_Current, 0); // Shoot_Can_Set_Current
                                                                                              // vTaskDelay(2);
...
static void GIMBAL_Feedback_Update(Gimbal_Control_t *gimbal_feedback_update)
{
    ...
    // printf("%.2f\r\n",gimbal_feedback_update->gimbal_pitch_motor.absolute_angle);
    ...
    //                                                                                     gimbal_feedback_update->gimbal_yaw_motor.offset_ecd);
    {
        ...
        // printf("%d %f %d\n",ecd,gimbal_feedback_update->gimbal_yaw_motor.relative_angle,gimbal_feedback_update->ecd_count);
    }
...
static void gimbal_motor_absolute_angle_control(Gimbal_Motor_t *gimbal_motor)
{
    ...
    /*SkyAdd-Start*/
    fp32 err = gimbal_motor->absolute_angle_set - gimbal_motor->absolute_angle;
    if (err > 180) {
        // error=-(360-error)
        err = err - 360;
    } else if (err < -180) {
    ...
    gimbal_motor->motor_gyro_set = GIMBAL_PID_Calc_WithErr(&gimbal_motor->gimbal_motor_absolute_angle_pid, gimbal_motor->absolute_angle, gimbal_motor->absolute_angle_set, err, gimbal_motor->motor_gyro);
    /*SkyAdd-End*/

    // gimbal_motor->motor_gyro_set = GIMBAL_PID_Calc(&gimbal_motor->gimbal_motor_absolute_angle_pid, gimbal_motor->absolute_angle, gimbal_motor->absolute_angle_set, gimbal_motor->motor_gyro);
    // printf("%.2f,%.2f\n",gimbal_motor->motor_gyro_set,gimbal_motor->motor_gyro);
...
static void gimbal_motor_relative_angle_control(Gimbal_Motor_t *gimbal_motor)
{
    ...
    /*SkyAdd-Start*/
    fp32 err = gimbal_motor->relative_angle_set - gimbal_motor->relative_angle;
    if (err > 180) {
        // error=-(360-error)
        err = err - 360;
    } else if (err < -180) {
    ...
    gimbal_motor->motor_gyro_set = GIMBAL_PID_Calc_WithErr(&gimbal_motor->gimbal_motor_relative_angle_pid, gimbal_motor->relative_angle, gimbal_motor->relative_angle_set, err, gimbal_motor->motor_gyro);
    /*SkyAdd-End*/
...
{
    gimbal_control.gimbal_yaw_motor.offset_ecd         = 6757;
    gimbal_control.gimbal_yaw_motor.max_relative_angle = PI / 3;
    gimbal_control.gimbal_yaw_motor.min_relative_angle = -PI / 3;

    gimbal_control.gimbal_pitch_motor.offset_ecd         = 6322 + 585;
    gimbal_control.gimbal_pitch_motor.max_relative_angle = 0.30;
