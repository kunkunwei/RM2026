...
extern dji_motor_measure_t pull_motor;
...
void shoot_init(void)
{
    ...
    //遥控器指针
    shoot_control.shoot_rc = &remote_ctrl;
    //电机指针
    shoot_control.shoot_motor_measure = &pull_motor;
    //初始化PID
    old_PID_Init(&shoot_control.trigger_motor_pid, PID_POSITION, Trigger_speed_pid, TRIGGER_READY_PID_MAX_OUT, TRIGGER_READY_PID_MAX_IOUT);
    //更新数据
    ...
    shoot_control.angle = shoot_control.shoot_motor_measure->pos * MOTOR_ECD_TO_ANGLE;
...
int16_t shoot_control_loop(void)
{
    ...
    {
        {
            old_PID_clear(&shoot_control.trigger_motor_pid);
    ...
    else
    {
        // shoot_laser_on(); //激光开启
        //计算拨弹轮电机PID
        old_PID_Calc(&shoot_control.trigger_motor_pid, shoot_control.speed, shoot_control.speed_set);
...
static void shoot_set_mode(void)
{
    ...
    //printf("%d,%d\r\n",shoot_control.shoot_rc->rc.ch[4],remote_ctrl.rc.ch[4]);
    //上拨判断， 一次开启，再次关闭
    ...
    if(switch_is_shoot(shoot_control.shoot_rc->rc.ch[4]) == false && (shoot_control.shoot_mode == SHOOT_CONTINUE_BULLET || shoot_control.shoot_mode == SHOOT_BULLET))
    {
        shoot_control.shoot_mode = SHOOT_READY;
    }
...
static void shoot_feedback_update(void)
{
    ...
    shoot_control.speed = shoot_control.shoot_motor_measure->rpm;
    ...
    if (shoot_control.shoot_motor_measure->pos - last_ecd > HALF_ECD_RANGE)
    {
        shoot_control.ecd_count--;
    }
    else if (shoot_control.shoot_motor_measure->pos - last_ecd < -HALF_ECD_RANGE)
    {
        shoot_control.ecd_count++;
    }
    last_ecd = shoot_control.shoot_motor_measure->pos;
    ...
    //计算输出轴角度
    shoot_control.angle = (shoot_control.ecd_count * ECD_RANGE + shoot_control.shoot_motor_measure->pos) * MOTOR_ECD_TO_ANGLE;
