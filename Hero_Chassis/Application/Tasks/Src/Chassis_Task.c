...
void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
    ...
    if(chassis_move_control->rc_control_mode->chassis_behaviour_mode == CHASSIS_NO_FOLLOW_GIMBAL_YAW){
        
    }
    else if(chassis_move_control->rc_control_mode->chassis_behaviour_mode == CHASSIS_FOLLOW_GIMBAL_YAW){
        //wan dian zai xie;
        //底盘跟随云台
        //chassis_rc_to_control_vector(&vx_set,&vy_set,&angle_set,chassis_move_control);
        //x,y依然使用遥控，wz需要使用PID计算
    }
    else if(chassis_move_control->rc_control_mode->chassis_behaviour_mode == CHASSIS_FORCE_RAW){
        vx_set = 0.0f;
        vy_set = 0.0f;
        angle_set = 0.0f;
    }
...
void chassis_control_loop(chassis_move_t *chassis_move_control_loop){
    ...
    if(chassis_move_control_loop->rc_control_mode->chassis_behaviour_mode == CHASSIS_FORCE_RAW){
            //赋值电流值
        for (uint8_t i = 0; i < 4; i++)
        {
