 ...
 */
CAN_TxFrameTypeDef GimbalTxFrame[2] = {
	[0]={
		.hcan = &hcan1,
...
CAN_TxFrameTypeDef ALLShootTxFrame ={
	.hcan = &hcan2,
	  .header.StdId=CAN2_CMD_ALL_ID,//0x1FF
	  .header.IDE=CAN_ID_STD,
	  .header.RTR=CAN_RTR_DATA,
	  .header.DLC=8,
};
CAN_TxFrameTypeDef ALLChassisTxFrame ={
	.hcan = &hcan1,	//改为hcan1了，本来是hcan2
	  .header.StdId=CAN1_CMD_ALL_ID,//0x200
	  .header.IDE=CAN_ID_STD,
	  .header.RTR=CAN_RTR_DATA,
	  .header.DLC=8,
};
...
static void CAN1_RxFifo0RxHandler(uint32_t *StdId, uint8_t data[8])
{
    ...
    switch(*StdId)
    {
		...
		case 0x203:
    	case 0x204:
			get_chassis_motor_measure(&chassis_motor[*StdId - 0x201], data);
			break;
    }
 }
...
static void CAN2_RxFifo0RxHandler(uint32_t *StdId, uint8_t data[8])
{
    ...
    case CAN2_YAW_MOTOR_ID:
    	get_dm_motor_measure(&yaw_motor, data);    // 达妙电机数据处理
    	break;
    case CAN2_PITCH_MOTOR_ID:
    	get_dji_motor_measure(&pitch_motor, data);
    	break;
    }
}
//------------------------------------------------------------------------------
