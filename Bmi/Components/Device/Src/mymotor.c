...
Chassis_power_measure_t chassis_power_measure;
...
void get_power_measure(Chassis_power_measure_t* ptr, uint8_t *rx_message)
{
    memcpy(&(ptr->power), rx_message, sizeof(float));
}
//返回指针
Chassis_power_measure_t* get_chassis_power_measure()
{
    return &chassis_power_measure;
}
