...
void calibrate_control(gimbal_t *gimbal, float *add_yaw, float *add_pitch)
{
    ...
    *add_pitch = 0.0f - gimbal->gimbal_pos.init_pitch_pos / 800.0f;
...
void gimbal_control_loop(gimbal_t *gimbal_loop)
{
    ...
    gimbal_loop->gimbal_pos.pitch_motor_measure->target_current =
        (int16_t)old_PID_Calc(&gimbal_loop->gimbal_pos.gimbal_motor_absolute_angle_pid,
    ...
    gimbal_loop->gimbal_pos.pitch_motor_measure->target_current -= (int16_t)gimbal_loop->gimbal_pos.pitch_absolute_pos * 15000.0f;
